...
async fn main() {
    ...
    let postgres_host = env::var("POSTGRES_HOST").unwrap_or_else(|_| {
        eprintln!("POSTGRES_HOST env variable is not set");
        exit(1)
    });
    let postgres_user = env::var("POSTGRES_USER").unwrap_or_else(|_| {
        eprintln!("POSTGRES_USER env variable is not set");
        exit(1)
    });
    let postgres_dbname = env::var("POSTGRES_DBNAME").unwrap_or_else(|_| {
        eprintln!("POSTGRES_DBNAME env variable is not set");
        exit(1)
    });
    ...
    let (client, connection) = tokio_postgres::connect(
        &format!(
            "host={} user={} dbname={}",
            postgres_host, postgres_user, postgres_dbname
        ),
        NoTls,
    )
    .await
    ...
    if config.full_scan_every.is_none() || config.rescan_every.is_none() {
        ...
        let full_scan_every = match config.full_scan_every {
            Some(dur) => {
                println!(
                    "scanning for new servers every {} hours",
                    dur.as_secs() / 3600
                );
                dur
            }
        ...
        match arg.as_str() {
            ...
            "-t" | "--timeout" => {
                ...
                config.timeout = timeout
                    ...
                    .expect("timeout is not a valid time")
                    .into()
            }
            "--rescan-every" => {
                ...
                config.rescan_every = Some(
                    time.parse::<humantime::Duration>()
                        .expect("time for '--rescan-every' is not a valid time")
                        .into(),
                )
            }
            "--full-scan-every" => {
                ...
                config.full_scan_every = Some(
                    time.parse::<humantime::Duration>()
                        .expect("time for '--full-scan-every' is not a valid time")
                        .into(),
                )
            }
            arg => {
...
async fn read_masscan_output(file_path: &str) -> Vec<SocketAddr> {
    ...
    let masscan_output = File::open(file_path)
        .await
        .expect("file in masscan/masscan-output.txt was not found, run with --masscan first");
    ...
    while let Some(line) = line_iter.next_line().await.unwrap() {
        ...
        let port: u16 = split
            .next()
            .expect("wrongly formatted masscan output")
            .parse()
            .expect("wrongly formatted masscan output");
...
async fn batch_server_list(
    server_list: Vec<SocketAddr>,
    tx: &Sender<Vec<(SocketAddr, Option<ServerInfo>, Option<ServerJoinInfo>)>>,
    config: &Config,
) {
    ...
    while i + config.batch_size < server_list_len {
        tx.send(scan_batch(&server_list[i..i + config.batch_size], &config).await)
            .await
            .unwrap();
        i += config.batch_size;
    ...
    if i != server_list_len {
        tx.send(scan_batch(&server_list[i..i + (server_list_len - i)], &config).await)
            .await
            .unwrap();
        i += server_list_len - i;
        println!("{i}/{server_list_len}");
...
async fn scan_batch(
    batch: &[SocketAddr],
    config: &Config,
) -> Vec<(SocketAddr, Option<ServerInfo>, Option<ServerJoinInfo>)> {
...
async fn scan_server(
    addr: SocketAddr,
    do_join_scan: bool,
) -> (SocketAddr, Option<ServerInfo>, Option<ServerJoinInfo>) {
    let Some(server_info) = ping_scan(addr).await else {
        return (addr, None, None);
    };

    if do_join_scan {
        let server_join_info = join_scan(
            addr,
            if server_info.protocol <= 0 {
                754
            } else {
...
async fn ping_scan(addr: SocketAddr) -> Option<ServerInfo> {
    ...
    stream
        .write_all(&util::prefix_len(&util::form_handshake(1, 754, addr)))
        .await
        .ok()?;
...
async fn join_scan(addr: SocketAddr, protocol: i32) -> Option<ServerJoinInfo> {
    ...
    stream
        .write_all(&util::prefix_len(&util::form_handshake(2, protocol, addr)))
        .await
        .ok()?;
    ...
    let login_start_fields = match protocol {
        ...
        759 => &[&util::prefix_len(username)[..], &[0x00]].concat(),
        760 => &[&util::prefix_len(username)[..], &[0x00, 0x00]].concat(),
    ...
    stream
        .write_all(&util::prefix_len(
            &[&[0x00], &login_start_fields[..]].concat(),
        ))
        .await
        .ok()?;
    ...
    match packet_id {
        0x00 => {
            ...
            match str {
                str if str.to_lowercase().contains("whitelist")
                    || str.to_lowercase().contains("white-list") =>
                {
                    Some(ServerJoinInfo {
                        cracked: Some(true),
                        whitelist: Some(true),
                ...
                str if str.to_lowercase().contains("forge") => Some(ServerJoinInfo {
                    cracked: Some(true),
                    whitelist: None,
                    forge: Some(true),
                }),
                _ => Some(ServerJoinInfo {
                    cracked: Some(true),
                    whitelist: None,
                    forge: None,
                }),
        ...
        0x01 => Some(ServerJoinInfo {
            cracked: Some(false),
            whitelist: None,
            forge: None,
        }),
        0x02 | 0x03 => Some(ServerJoinInfo {
            cracked: Some(true),
